...
@Component({
  selector: 'add-story-reactive',
  templateUrl: './addstory.reactive.component.html'
})
export class AddStoryReactiveComponent {
  form: FormGroup;
  story: Story;
  ...
  onSubmit() {
    if (this.isFormReady()) {
      ...
      for (let i = 0; i < this.localImgSrcs.length; i++) {
        if(this.localImgSrcs[0].file){
          this.story.localImages.push(this.localImgSrcs[0].file);
        }
      }
